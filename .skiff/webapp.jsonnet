...
// Generate the ingress path entry for the given model
local targeted_attack_path(model_name) = {
    path: '/targeted_attack/' + model_name,
    backend: {
        serviceName: fullyQualifiedName + '-' + model_name,
        servicePort: config.httpPort
    },
        ...
        rules: [
            {
                ...
                http: {
                    paths: std.flattenArrays([
                        [predict_path(model_name), interpret_path(model_name), attack_path(model_name), targeted_attack_path(model_name)]
